...
-- | Adds an Anime, either through arguments or interactive entering
--
completelyNew :: [String] -> IO ()
completelyNew []   = undefined -- prompt ...>
completelyNew args
  | elem "-i" args = loadList >>= saveComplete . insert $ (newName (singleton uiD) (T.pack name) )
  where filt = filter (\e -> e /= "-i") args
        uiD  = last filt
        name = unwords . init $ filt
  | otherwise      = loadList >>= (\col -> saveComplete . insert $ (newName (singleton (maxId col) ) (T.pack name) ) )
  where name = unwords args
-}
